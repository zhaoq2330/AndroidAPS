...
class SyaiWorkerTest : TestBaseWithProfile() {

    private lateinit var worker: SyaiPlugin.SyaiWorker
    @Mock lateinit var syaiPlugin: SyaiPlugin
    ...
    fun `When plugin disabled then return success`() {
        runBlocking {
            whenever(syaiPlugin.isEnabled()).thenReturn(false)
    ...
    fun `When plugin enabled then insert data`() {
        ...
        runBlocking {
            whenever(syaiPlugin.isEnabled()).thenReturn(true)
            ...
            verify(persistenceLayer).insertCgmSourceData(Sources.SyaiTag, listOf(expectedGv), emptyList(), null)
        }
    }
